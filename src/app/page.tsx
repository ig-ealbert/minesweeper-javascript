...
import { adjacentCellsWithMines } from "@/lib/hints";
...
export default function Home() {
  ...
  const [mineLayout, setMineLayout] = useState<mineState>(initializeGameState(size, numMines));
  useEffect(() => setMineLayout(initializeGameState(size, numMines)), [size, numMines]);
  ...
  function reset() {
    ...
    const empty = new Array(size.rows).fill(
      new Array(size.columns).fill(0)
    );
  ...
  return (
    <div className={styles.page}>
      <main className={styles.main}>
        <Board rows={size.rows} columns={size.columns}
               boardValue={boardValue} boardStatus={boardStatus}
               setBoardValue={handleUpdateValues} setBoardStatus={handleUpdateStatuses}
        ...
        <div>
          <select id="level" value={difficulty} 
                  className={styles.option} onChange={handleDifficultyChange}>
            <option value="easy">Easy</option>
            <option value="medium">Medium</option>
            <option value="hard">Hard</option>
          </select>
          <input id="restartGame" type="button" value="Reset" disabled={!isGameOver}
                 className={styles.option} onClick={reset} />
         </div>
        <div>
